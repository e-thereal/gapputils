...
BeginPropertyDefinitions(OneOfN)
  ...
  WorkflowProperty(Input, Input("I"))
  WorkflowProperty(Inputs, Input("Is"))
...
void OneOfN::update(IProgressMonitor* monitor) const {
  ...
  if (getInput() && getInput()->size()) {
    data_t& input = *getInput();

    double minimum = input[0], maximum = input[0];
  ...
  } else if (getInputs() && getInputs()->size()) {
